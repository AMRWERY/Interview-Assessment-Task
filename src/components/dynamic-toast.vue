    ...
    <div>
        <transition name="bounce" mode="out-in">
            <div v-if="visible"
                ...
                <iconify-icon :icon="toastIcon" width="24" height="24":class="`w-6 h-6 ${iconColor}`" class="inline shrink-0 me-3"></iconify-icon>
                <span class="block text-lg me-3 sm:inline">{{ message }}</span>
                    <iconify-icon icon="material-symbols:close-small-rounded" width="24" height="24" class="inline w-6 h-6 cursor-pointer ms-auto shrink-0" @click="closeToast"></iconify-icon>
            </div>
        </transition>
    </div>
</template>
